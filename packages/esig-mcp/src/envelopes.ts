...
export interface EnvelopeStatusSummary {
  ...
  completedAt?: string;
  voidedAt?: string;
  /** Set once the envelope reaches `completed` and the seal step has run. */
  sealedPdfUrl?: string;
}
...
class HourlyRateLimiter {
  ...
  take(): void {
    ...
    if (this.hits.length >= this.limit) {
      throw new EnvelopeError(`hourly envelope-creation cap reached (${this.limit}/hour)`);
    }
...
export class EnvelopeService {
  ...
  /** Approval-page-facing: record a drawn signature; seals the document when this was the last signer. */
   ...
   * audit. Runs once, right after the last signer completes an envelope.
   */
    ...
    await this.deps.auditStore.insert({
      ...
      metadata: {
        post_quantum: result.pqSealed
          ? { sealed: true, alg: "hybrid-ed25519-ml-dsa-65", key_id: result.pqKeyId, mldsa65_fpr: result.pqMldsa65Fpr }
          : { sealed: false },
      },
    });
